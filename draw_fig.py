...
class DrawFig:
    ...
    def draw_value(self, xaxis, label, value_name, label_vals=[], yscale:str="linear", approx:bool=False):
        fig, ax = plt_func.create_subplots(figsize=(3,2))
        ...
        for value_arr in value_tot:
            if label_vals == [] or value_arr['label'] in label_vals:
                #ax.errorbar(value_arr['xaxis'], value_arr['mean'], yerr=value_arr['std'], marker='o', capthick=1, capsize=2, label=self.label[label]+f'={value_arr['label']}', color=cmap(i/label_num), linestyle="None", zorder=i, markersize=1)
                ax.plot(value_arr['xaxis'], value_arr['mean'],marker='o',label=self.label[label]+f'={value_arr['label']}', color=cmap(i/label_num), linestyle="None", zorder=i, markersize=2)
                if approx:
                    rhos, Js = analysis.approx_fdiagram(qmax=i)
                    ax.plot(rhos, Js, color=cmap(i/label_num), linestyle="dashed", zorder=i, linewidth=0.6)
                i+=1
        # for qmax in [2,3,4]:
        ...
        self.save(fig=fig, name=name, style="svg")

if __name__ == '__main__':
    file_name = 'fitting_alpha'
    func.create_directory(path='image/'+file_name)
    plt_func.plt_setting()
    df = DrawFig(file=file_name)
    #df.params_def["rho"] = Fraction(1,3)
    df.draw_value(xaxis='rho', label='beta', value_name='hop_rate',approx=True)
